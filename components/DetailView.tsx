...
export default function DetailView({ tipo, id }: { tipo: MediaType; id: string }) {
  ...
  const heroBg = t.poster ? { backgroundImage: `url(${t.poster})` } : { background: "#2A2D33" };
  ...
  return (
    <div className="detail-inner">
      ...
      <div className="dpad">
        ...
        {mine.length ? (
          <a className="dprimary" href={t.links[mine[0]] ?? "#"} target="_blank" rel="noreferrer">
        ...
        <div className="actions">
          <button className={`act ${inList ? "on" : ""}`} onClick={() => setInList((v) => !v)}>
            <svg viewBox="0 0 24 24" strokeLinecap="round">{inList ? <path d="M5 12l5 5 9-12" /> : <path d="M12 5v14M5 12h14" />}</svg>
            <span className="lab">Mi lista</span>
          </button>
          <button className={`act ${rated ? "on" : ""}`} onClick={() => setRated((v) => !v)}>
            <svg viewBox="0 0 24 24" strokeLinecap="round" strokeLinejoin="round"><path d="M7 10v11M7 10l4-7c1.5 0 2.5 1 2.5 2.5L13 10h5.5c1.1 0 2 1 1.8 2.1l-1.3 7c-.2 1-1 1.9-2 1.9H7" /></svg>
            <span className="lab">Calificar</span>
          </button>
          <button className="act" onClick={() => navigator.share?.({ title: t.title }).catch(() => {})}>
            <svg viewBox="0 0 24 24" strokeLinecap="round" strokeLinejoin="round"><circle cx="18" cy="5" r="3" /><circle cx="6" cy="12" r="3" /><circle cx="18" cy="19" r="3" /><path d="M8.6 13.5l6.8 4M15.4 6.5l-6.8 4" /></svg>
            <span className="lab">Compartir</span>
          </button>
        </div>

        {t.synopsis && <p className="dsyn">{t.synopsis}</p>}
        {t.cast.length > 0 && <p className="dcast"><b>Reparto:</b> {t.cast.slice(0, 4).join(", ")}{t.cast.length > 4 ? "… más" : ""}</p>}
        {t.directors.length > 0 && <p className="dcast"><b>Dirección:</b> {t.directors.join(", ")}</p>}

        {(t.imdb != null || t.metacritic != null || t.editorial) && (
          <>
            ...
            <div className="rating-bar">
              ...
              {t.editorial?.rating != null && <div className="rb ed"><div className="lbl">Reseña</div><div className="num">{t.editorial.rating.toFixed(1)}</div></div>}
            </div>
          </>
        )}
        ...
        {t.editorial && (
          <>
            ...
            <div className="review">
              <div className="badge">{star}StreamingCentral</div>
