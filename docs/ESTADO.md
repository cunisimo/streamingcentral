...
| **Capacitor** | **ETAPA 2 TERMINADA (CP1-CP11).** Veredicto: viable en Android, se recomienda avanzar. El spike descartable queda archivado en `spike/capacitor-android` (`5d5180f`). ✅ **ETAPA 3 TERMINADA** (4 tandas) en `integracion/capacitor-base`, nacida de `main`, **sin pushear y sin mergear**. **Etapa 4 EN CURSO**: íconos y pantalla de inicio nativos ya aplicados y aprobados (6/09) — el ícono lleva sólo el símbolo y el splash lleva la palabra "yump", en claro y oscuro, con capa monochrome. Probados en el teléfono el 6/09: **el ícono está bien**; ✅ **la pantalla de inicio también, corregida el 6/09**: se declararon los atributos que la SplashScreen API sí mira y ahora muestra la marca completa con "yump" en claro y en oscuro, sin símbolo suelto y sin destello. Los splash.png heredados se conservan para Android 11 y anteriores. Corregida además la documentación: hay DOS fuentes de marca y DOS generadores (web y Android), y logo.svg no lo lee nadie. Primera tanda (5/09): `noindex` en `/t` y `/p`, `capacitor.config.ts` y `android/` generados con `ar.yump.app`. Segunda tanda (6/09): sólo diagnóstico — el APK `ar.yump.app` se instaló y pasó las regresiones, y quedaron medidos el `.ics`, las subrutas, Analytics y `sc_platforms`. Tercera tanda (6/09): Analytics y Speed Insights apagados sólo en nativo (cero pedidos, antes 2 con 404) y "Recordarme" en Android va derecho a Google Calendar sin `.ics`; la alternativa B para subrutas se probó entera y **se revirtió** porque empeora el problema. Cuarta tanda (6/09): **las subrutas quedaron resueltas** desde el cliente — el build nativo inyecta en el artefacto un guion que recupera la ruta directa con su query y su hash, sin tocar la web. Queda pendiente que el 404 devuelva un HTTP 404 real (hoy se ve pero responde 200) y que sea una página propia. 🔴 **Próximamente y "¿No sabés qué ver?" tienen cambios que TODAVÍA NO se integraron a esta rama** y hay que integrarlos antes del paquete publicable. Ver `docs/superpowers/plans/2026-09-05-etapa3-android-publicable.md` |
