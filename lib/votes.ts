...
// Top de títulos por cantidad de likes en los últimos `days` días.
export async function topVotedRows(days = 7, limit = 60): Promise<VotedRow[]> {
  const sb = supabaseServer();
  if (!sb) return [];
  const { data, error } = await sb.rpc("top_voted", { p_days: days, p_limit: limit });
